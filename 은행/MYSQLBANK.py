...
def deposit_in_and_out(account, account1, balance):
    try:
        sql = "SELECT 계좌번호, 예금주, 잔액 FROM 입출금통장 WHERE 계좌번호=%s ORDER BY 거래일 DESC LIMIT 1"
        sql_in = "INSERT INTO 입출금통장 (계좌번호, 예금주, 입금액, 보낸분_받는분, 잔액) VALUES(%s,%s,%s,%s,%s)" 
        data1 = []
        with conn:
            with conn.cursor() as cur:
                cur.execute(sql, (account))
                cur.execute(sql, (account1))
                result = cur.fetchall()
                for data in result:
                    data1 = data1 + list(data)
                sum1 = int(data1[2])-int(balance)
                sum2 = int(data1[5])+int(balance)
                cur.execute(sql_in, (account, data1[1], balance, data1[4], sum1))
                cur.execute(sql_in, (account1, data1[4], balance, data1[1], sum2))
                conn.commit()
    except Exception:
        print("잘못된 정보입니다.")
